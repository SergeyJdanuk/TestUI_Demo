...
(function () {
    ...
    var host = 'http://api.megogo.net';
    var api = 'v1';
    var public_key = '_dune_j1';
    var private_key = '78ae72ce0f';
    ...
    function getSign() {
        var sign = md5(options.replace(new RegExp("&", "g"), '') + private_key);
        return sign + public_key;
    }

    function getApiUrl(options, type) {
        if (!options) {
            options = '';
        }

        options = options + '&lang=ru&did=0000-0000-7FAC-02AA-2600-1915-47EB-159B_tv102';
        var sign = md5(options.replace(new RegExp("&", "g"), '') + private_key);
        var api_url = host + '/' + api + '/' + type + '?' + options + '&sign=' + sign + public_key;
        return api_url;
    }

    function generateId() {
        var deviceID = 'xxxxxxxxZxxxxZ4xxxZyxxxZxxxxxxxxxxxx'.replace(/[xy]/g, function (c) {
    ...
    var Controller = Supplier_IDriver.extend({
        ...
        getMovieMediaUrl: function (movie_id, callback) {
            console.log('megogo: getting media url');
            return Zepto.ajax({
              type: 'GET',
              url: getApiUrl('video_id='+movie_id,'stream'),
        ...
        getChannelMediaUrl: function (channel_id) {

        }
    });
   window.Supplier_Megogo = Controller;
})();
